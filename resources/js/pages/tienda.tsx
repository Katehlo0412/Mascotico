      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {productos.map(producto => (
            <Link key={producto.id} href={`/producto/${producto.id}`} className="w-full">
              <div
                className="bg-white rounded-xl shadow-md p-3 flex flex-col items-center border border-yellow-100 relative group w-64 mx-auto transition-transform duration-300 hover:scale-105 hover:shadow-2xl cursor-pointer"
                style={{ minHeight: 320 }}
              >
